...
// per-action views in views/. collector hooks marked "DATA" in each view.
...
#define ANIM_TICK_MS 33u // 30fps redraw
...
static void drop_detect_submenu_callback(void* context, uint32_t index) {
    ...
    // session anchors live in the target view's enter hook, not here
    switch(index) {
...
/* 33ms tick - global frame counter, fans out to every view */
...
static DropDetectApp* drop_detect_app_alloc(void) {
    ...
    view_dispatcher_set_navigation_event_callback(
        app->view_dispatcher, drop_detect_navigation_event_callback);
    view_dispatcher_set_custom_event_callback(
        app->view_dispatcher, drop_detect_custom_event_callback);

    app->submenu = submenu_alloc();
    drop_detect_submenu_setup(app);
    view_dispatcher_add_view(
        app->view_dispatcher, DropDetectViewSubmenu, submenu_get_view(app->submenu));

    app->drop_view = drop_view_alloc();
    drop_view_set_notification(app->notif); // wire the "DROP!" cue
    ...
    // imu + sampler timer hook up here.
    // views start/stop the sampler from their enter/exit anchors.
...
int32_t drop_detect_app(void* p) {
    ...
    view_dispatcher_switch_to_view(app->view_dispatcher, DropDetectViewSubmenu);
